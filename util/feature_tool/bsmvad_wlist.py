    ...
    for datatype in ['v', 'a', 'd']:
            for analysis_type in ['mean', 'std', 'ratio']:
                for uniq_status in ['uniq_', '']:
                    dict[f"BsmVAD_glob_{datatype}_{uniq_status}{analysis_type}"] = 0.0
    for datatype in ['v', 'a', 'd']:
        for analysis_type in ['max_word', 'min_word', 'most_word']:
            dict[f"BsmVAD_glob_{datatype}_{analysis_type}"] = 0.0
    return dict
        ...
        for analysis_type in ['means', 'stds']:
            for uniq_status in ['_uniq', '']:
                for value in ['mean', 'std']:
                    dict[f'BsmVAD_{datatype}{uniq_status}_{analysis_type}_{value}'] = 0.0
    for datatype in ['v', 'a', 'd']:
        for analysis_type in ['max_words', 'min_words', 'most_words']:
            for value in ['mean', 'std']:
                dict[f'BsmVAD_{datatype}_{analysis_type}_{value}'] = 0.0
    return dict
...
class BSMVAD_wlist:
    ...
    def wordlevel_analysis(self, song_df, glob_df) -> dict:
        if(len(song_df) > 0):
            matched_word_df = unsquished_intersection(song_df, self.bsmvad_df)
            uniq_matched_word_df = glob_intersection(glob_df, self.bsmvad_df)
    ...
    def process_comment(self, index, comment):
        ...
        found_uniq_word_df = glob_intersection(unique_words_df, self.bsmvad_df)
        found_words_df = pd.merge(words_df, self.bsmvad_df, on='Word')
    ...
    def _itervad(self):
        for data_type in [('v', 'V.Mean.Sum'), ('a', 'A.Mean.Sum'), ('d', 'D.Mean.Sum')]:
